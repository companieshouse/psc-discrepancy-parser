...
The scripts consist of 5 modules, namely: lambda-roles, lambda, s3, security-group, ses.

- Lambda-Roles: This module consists of the permissions the Lambda function needs. The majority of the
permissions are regarding logs and cloudwatch, the s3 bucket for the email storage and ec2 for VPC and network config.
- Lambda: Consists of the configuration needed for the Lambda function in AWS. E.g. memory size, s3 bucket, trigger,
handler function, etc. 
- S3: Configuration required to create the S3 bucket for the storage of the emails. The bucket is made completely private,
and there is also a policy to allow SES to forward emails to the S3 bucket.
- Security-Group: Gives access to the Lambda function for the internal VPC in order to send data over to CHIPS.
- SES: Creates a rule on the active rule set for SES in order to forward any incoming psc discrepancy report emails onto
S3.

Each module has a set of variables that can be found in the `variables.tf` file - these variables are supplied by the
vars file in the `environments` directory found in the root of the `terraform` directory. The variables include config
such as the parent state file, the name of the S3 bucket for the emails, the SES rule set name, etc.

##### Note:
- The variable for `networks_state_bucket` only exists because platform have been moving parent state files into appropriate
  buckets. Therefore, at this point in time (December 13, 2019) the statefile generated by the Terraform scripts in this
  project will live in a separate bucket than the parent statefiles for CH. For future projects, there should be only one
  bucket for the statefile for the project and the parent statefiles (with the state prefix being different).
- The SES module depends on there being an active email ruleset in the specific environment otherwise the `plan` would fail.

The infrastructure that is set up is not environment-specific (e.g. only available on Honeydew) - it is global. However,
there is an environment variable that is specific to the local CHIPS servers in CH. This variable only points to the URL
that the CHIPS REST Interface is deployed to on Honeydew, ENV14, Live, etc.
