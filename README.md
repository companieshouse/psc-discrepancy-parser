...
Where possible, changes should be deployed via Concourse.
...
### Design
The terraform code consist of 5 modules:
...
Each module has a set of variables that can be found in the variables.tf file - these variables are defined in the `vars` file on a per-region and per-environment basis. E.g. `profiles/devleopment-eu-west-1/dev/vars` The variables include config such as the parent state file, the name of the S3 bucket for the emails, the SES rule set name, etc.
